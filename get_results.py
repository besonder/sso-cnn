...
df = pd.DataFrame(columns=['backbone', 'conv', 'linear', 'eps', 'std', 'lr max', 'epochs', 'seed', 
                           'test acc', 'emp lip', 'robust acc', 'cert robust', 'train acc', 'exp name'])
...
agg = df.groupby('exp name').aggregate({
    ...
    'emp lip': 'mean', 'robust acc': 'mean', 'cert robust': 'mean'})
agg.rename(columns={'exp name': 'count'}, inplace=True)
agg.backbone = agg.backbone.apply(lambda x: x.split()[0])
agg.conv = agg.conv.apply(lambda x: x.split()[0])
agg.linear = agg.linear.apply(lambda x: x.split()[0])
agg['std'] = agg['std'].astype(bool)

agg = agg.sort_values(by = ['backbone', 'test acc'], ascending=False)
print(agg.reset_index(drop=True))