...
class CayleyConv2d(StridedConv2d, nn.Conv2d):
    ...
    def forward(self, x):
        ...
        yfft = (Cayley(self.alpha * wfft / wfft.norm()) @ xfft).reshape(n, n // 2 + 1, self.out_channels, batches)
    ...
    def genFreqSES(self, n):
        ...
        for i in range(150):
            ...
            if self.cout >= self.xcin:
                ...
                RowNorm = torch.norm(Hfft, dim=2)
                RowLoss = loss(RowNorm, torch.ones_like(RowNorm, dtype=RowNorm.dtype))
                ColNorm = torch.norm(Hfft, dim=1)
                ColLoss = loss(ColNorm, torch.ones_like(ColNorm, dtype=ColNorm.dtype)*np.sqrt(self.cout/self.xcin))
...
class SESLinear(nn.Module):
    ...
    def genSpacSES(self):
        ...
        for i in range(300):
            ...
            if self.cout >= self.xcin:
                RowNorm = torch.norm(H, dim=1)
                RowLoss = loss(RowNorm, torch.ones_like(RowNorm, dtype=RowNorm.dtype)*np.sqrt(self.xcin/self.cout)*scale)
                ColNorm = torch.norm(H, dim=0)
                ColLoss = loss(ColNorm, torch.ones_like(ColNorm, dtype=ColNorm.dtype)*scale)
                HHT = H @ H.T
                M = torch.triu(torch.ones((self.cout, self.cout), dtype=bool), diagonal=1).to(H.device)
                RowAngLoss = loss(HHT[M], torch.zeros_like(HHT[M], dtype=HHT[M].dtype))
                L = self.cout*ColLoss/2 + self.xcin*RowLoss + RowAngLoss
            else:
                RowNorm = torch.norm(H, dim=1)
                RowLoss = loss(RowNorm, torch.ones_like(RowNorm, dtype=RowNorm.dtype)*scale)
                ColNorm = torch.norm(H, dim=0)
                ColLoss = loss(ColNorm, torch.ones_like(ColNorm, dtype=ColNorm.dtype)*np.sqrt(self.cout/self.xcin)*scale)
                HTH = H.T @ H
                M = torch.triu(torch.ones((self.xcin, self.xcin), dtype=bool), diagonal=1).to(H.device)
                ColAngLoss = loss(HTH[M], torch.zeros_like(HTH[M], dtype=HTH[M].dtype))
                L = self.cout*ColLoss + self.xcin*RowLoss + ColAngLoss
